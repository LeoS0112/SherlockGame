...
from backend.game.stability import get_image_tile, stability_use
import os
import sys
...
class Character:
    def __init__(
        self, name, description, usefulness, sherlock_logic, weapon=None, item=None
    ):
        self.name = pre_parse_names(name)
        self.description = description
        ...
        if os.path.exists(out_path):
            ...
            person_path = stability_use(
                out_path,
                f"Create a character called {self.name} with a description of {self.description}",
            )
